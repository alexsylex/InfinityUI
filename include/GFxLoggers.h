...
namespace IUI
{
	...
	public:
		...
		void LogMembersOf(const RE::GFxValue& a_value) const
		{
			...
			if (a_value.IsObject())
			{
				...
				a_value.VisitMembers(const_cast<GFxMemberLogger*>(this));
				logger::at_level(logLevel, "{}", "}");
			}
			logger::flush();
		}

		void LogMembersOf(RE::GFxMovieView* a_view, const char* a_pathToMember) const
		{
			if (a_view) 
			{
				RE::GFxValue value;
	...
	public:
		...
		void LogElementsOf(const RE::GFxValue& a_value) const
		{
			logger::at_level(logLevel, "{}: {}", a_value.ToString().c_str(), GFxValueTypeToString(a_value.GetType()));
			if (a_value.IsArray())
			{
				logger::at_level(logLevel, "{}", "{");
				a_value.VisitElements(const_cast<GFxArrayLogger*>(this));
				logger::at_level(logLevel, "{}", "}");
			}
			logger::flush();
		}
		void LogElementsOf(RE::GFxMovieView* a_view, const char* a_pathToMember) const
		{
			if (a_view)
			{
				RE::GFxValue value;
